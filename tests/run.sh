...
test_workspace_model_start_disables_mtp_for_reliable_recovery() {
  ...
  ' _ "$SPARK")
  rm -rf "$tmp"
  [[ "$out" == *"Org/Model --no-mtp --tools"* ]] && [[ "$out" != *"--max-len"* ]] && [[ "$out" != *"--no-wait"* ]]
}
...
test_workspace_model_restarts_without_tool_calling() {
  ...
  ' _ "$SPARK")
  rm -rf "$tmp"
  [[ "$out" == *"Org/Model --no-mtp --tools --force"* ]] && [[ "$out" != *"--max-len"* ]]
}
...
test_workspace_doctor_json() {
  ...
  printf '%s' "$out" | jq -e '
    ...
    ([.areas[] | select(.name == "Inference & agent" and .passed == 17 and .failed == 0)] | length == 1) and
