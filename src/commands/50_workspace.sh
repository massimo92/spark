...
workspace_normalize_hermes_container_name() {
  local current
  current=$(workspace_hermes_container_name 2>/dev/null || true)
  [[ -n "$current" && "$current" != "$WORKSPACE_HERMES_CONTAINER" ]] || return 0
  if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qx "$WORKSPACE_HERMES_CONTAINER"; then
    warn "Hermes container name already in use: ${WORKSPACE_HERMES_CONTAINER}"
    return 0
  fi
  docker rename "$current" "$WORKSPACE_HERMES_CONTAINER" >/dev/null 2>&1 ||
    warn "Could not rename Hermes container to ${WORKSPACE_HERMES_CONTAINER}"
}
