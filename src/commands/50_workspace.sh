...
workspace_write_files() {
  ...
  workspace_require_env_value "Vikunja human password" "$human_pass"
...
workspace_reconcile_identity_overrides() {
  ...
  if [[ -n "${SPARK_WORKSPACE_VIKUNJA_PASSWORD:-}" && -n "${SPARK_WORKSPACE_N8N_PASSWORD:-}" \
    && "$SPARK_WORKSPACE_VIKUNJA_PASSWORD" != "$SPARK_WORKSPACE_N8N_PASSWORD" ]]; then
    warn "Using Workspace password for both Vikunja and n8n"
    SPARK_WORKSPACE_N8N_PASSWORD="$SPARK_WORKSPACE_VIKUNJA_PASSWORD"
  fi
  ...
  [[ -z "${SPARK_WORKSPACE_N8N_EMAIL:-}" && -n "${SPARK_WORKSPACE_VIKUNJA_EMAIL:-}" ]] \
    && SPARK_WORKSPACE_N8N_EMAIL="$SPARK_WORKSPACE_VIKUNJA_EMAIL"
  [[ -z "${SPARK_WORKSPACE_VIKUNJA_PASSWORD:-}" && -n "${SPARK_WORKSPACE_N8N_PASSWORD:-}" ]] \
    && SPARK_WORKSPACE_VIKUNJA_PASSWORD="$SPARK_WORKSPACE_N8N_PASSWORD"
  [[ -z "${SPARK_WORKSPACE_N8N_PASSWORD:-}" && -n "${SPARK_WORKSPACE_VIKUNJA_PASSWORD:-}" ]] \
    && SPARK_WORKSPACE_N8N_PASSWORD="$SPARK_WORKSPACE_VIKUNJA_PASSWORD"
  return 0
}
...
workspace_setup() {
  ...
  [[ -z "$remote_spec" ]] || {
    if [[ "$check_only" != "1" ]]; then
      ...
      SPARK_WORKSPACE_N8N_EMAIL="$SPARK_WORKSPACE_VIKUNJA_EMAIL"
      SPARK_WORKSPACE_VIKUNJA_PASSWORD=$(workspace_prompt_secret_confirm SPARK_WORKSPACE_VIKUNJA_PASSWORD "Workspace password")
      SPARK_WORKSPACE_N8N_PASSWORD="$SPARK_WORKSPACE_VIKUNJA_PASSWORD"
  ...
  local human_user human_email human_pass n8n_email n8n_pass hermes_pass workspace_pass
  ...
  if [[ -n "${SPARK_WORKSPACE_VIKUNJA_PASSWORD:-}" || -n "${SPARK_WORKSPACE_N8N_PASSWORD:-}" ]]; then
    workspace_reconcile_identity_overrides
    workspace_pass=$(workspace_prompt_secret_confirm SPARK_WORKSPACE_VIKUNJA_PASSWORD "Workspace password")
  elif [[ -n "$existing_n8n_pass" ]]; then
    workspace_pass="$existing_n8n_pass"
  else
    workspace_pass=$(workspace_prompt_secret_confirm SPARK_WORKSPACE_VIKUNJA_PASSWORD "Workspace password")
  fi
  human_pass="$workspace_pass"
  n8n_pass="$workspace_pass"
  SPARK_WORKSPACE_VIKUNJA_PASSWORD="$workspace_pass"
  SPARK_WORKSPACE_N8N_PASSWORD="$workspace_pass"
  ...
  workspace_require_prompt_value "Workspace password" "$workspace_pass" text
