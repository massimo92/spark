...
workspace_no_legacy_recovery_config() {
  ...
  for file in "$WORKSPACE_ENV_FILE" "$WORKSPACE_VIKUNJA_ENV_FILE" "$WORKSPACE_N8N_ENV_FILE"; do
    [[ -f "$file" ]] || return 1
    ! grep -Eq '^(WORKSPACE_SMTP_|VIKUNJA_MAILER_|N8N_SMTP_|N8N_EMAIL_MODE=|N8N_INSTANCE_OWNER_(MANAGED_BY_ENV|EMAIL|FIRST_NAME|LAST_NAME|PASSWORD_HASH)=)' "$file" || return 1
  done
}
...
workspace_vikunja_human_user_id() {
  ...
  email=$(workspace_read_env VIKUNJA_HUMAN_EMAIL 2>/dev/null || true)
  [[ -n "$username" && -n "$email" ]] || return 1
  workspace_vikunja_cli user list 2>/dev/null | awk -F'|' -v username="$username" -v email="$email" '
    function trim(s) { gsub(/^[ \t]+|[ \t]+$/, "", s); return s }
    NF >= 5 && trim($3) == username && trim($4) == email { print trim($2); exit }
  '
}
...
workspace_recover_n8n() {
  ...
  if [[ "$rc" == "0" ]]; then
    workspace_n8n_login "$email" "$password" || rc=1
  fi
