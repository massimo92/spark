...
workspace_create_vikunja_users() {
  ...
  workspace_create_vikunja_hermes_token "$jwt" "$WORKSPACE_VIKUNJA_HERMES_BOT_ID_RESULT" || {
    warn "Create a token for bot-hermes under Settings > Bots, then rerun with --vikunja-token"
    return 1
  }
  token=$(workspace_read_env VIKUNJA_HERMES_API_TOKEN 2>/dev/null || true)
...
workspace_print_initial_credentials() {
  ...
  if [[ -n "${WORKSPACE_SUPERSYNC_PASSKEY_ENROLLMENT_URL:-}" ]]; then
    [[ "$shown" == "0" ]] || printf "\n"
    printf "  SuperSync passkey enrollment\n"
    printf "    email: %s\n" "$vikunja_email"
    printf "    link:  %s\n" "$WORKSPACE_SUPERSYNC_PASSKEY_ENROLLMENT_URL"
    printf "\n  Open this one-time link within 15 minutes and create the passkey on your device.\n"
    shown=1
  fi
  if [[ "$shown" != "0" ]]; then
    if [[ "$(workspace_task_manager)" == "vikunja" ]]; then
