...
cmd_update() {
  ...
  local tailscale_update=0 tailscale_version=""
  ...
  # --- Tailscale self-update ---
  if command -v tailscale >/dev/null 2>&1; then
    tailscale_version=$(tailscale version 2>/dev/null | awk 'NR==1 {print $1}')
    tailscale_update=1
  fi

  update_count=$(( spark_update + ngc_update + gateway_update + postgres_update + vikunja_update + n8n_update + nemohermes_update + tailscale_update ))
  ...
  [[ "$tailscale_update" -eq 1 ]] && printf "    - Tailscale self-update check%s\n" "${tailscale_version:+ (current ${tailscale_version})}"
  printf "\n"
  ...
  if [[ "$nemohermes_update" -eq 1 ]] && confirm "Update NemoHermes sandbox?"; then
    NEMOCLAW_SANDBOX_NAME=hermes nemohermes hermes rebuild \
      && info "NemoHermes sandbox updated" \
      || err "Failed to update NemoHermes sandbox"
    did_update=1
  fi

  if [[ "$tailscale_update" -eq 1 ]] && confirm "Run Tailscale self-update?"; then
    tailscale update \
      && info "Tailscale update completed" \
      || err "Failed to update Tailscale"
    did_update=1
  fi
...
cmd_help() {
  ...
  ${BOLD}Commands:${NC}
    ...
    update           Check and apply Spark, model, workspace, and tool updates
