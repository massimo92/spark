...
cmd_run_help() {
  ...
  Aliases accept any other flag for one run only: Spark settings (--mem,
  --max-len, --max-num-seqs, --kv-cache-dtype, --enforce-eager) or any vLLM
  argument, e.g. --kv-cache-memory-bytes 12G. The saved alias is unchanged;
  save a change with: spark alias set <alias> ... Launch policy flags stay
  fixed (--tools, --text-only, --no-reasoning, --mtp, --no-mtp,
  --no-mem-limit, --regen-profile, --no-pull); change them with alias edit.
...
cmd_run() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --mem)       [[ $# -ge 2 ]] || die "Missing value for --mem"; mem="$2"; alias_setting_args+=(--mem "$2"); bundle_explicit_run_args+=(--mem "$2"); shift 2 ;;
      --no-mem-limit) no_mem_limit=1; alias_frozen_flags=1; bundle_explicit_run_args+=(--no-mem-limit); shift ;;
      --enforce-eager)    enforce_eager_flag=1; alias_setting_args+=(--enforce-eager); bundle_explicit_run_args+=(--enforce-eager); shift ;;
      --no-enforce-eager) enforce_eager_flag=0; alias_setting_args+=(--no-enforce-eager); bundle_explicit_run_args+=(--no-enforce-eager); shift ;;
      --no-wait)   no_wait=1; alias_override_args+=(--no-wait); shift ;;
      --max-num-seqs) [[ $# -ge 2 ]] || die "Missing value for --max-num-seqs"; max_num_seqs="$2"; alias_setting_args+=(--max-num-seqs "$2"); bundle_explicit_run_args+=(--max-num-seqs "$2"); shift 2 ;;
      --max-len)   [[ $# -ge 2 ]] || die "Missing value for --max-len"; max_len="$2"; alias_setting_args+=(--max-len "$2"); bundle_explicit_run_args+=(--max-len "$2"); shift 2 ;;
      --port)      [[ $# -ge 2 ]] || die "Missing value for --port"; port="$2"; alias_override_args+=(--port "$2"); bundle_explicit_run_args+=(--port "$2"); shift 2 ;;
      --kv-cache-dtype) [[ $# -ge 2 ]] || die "Missing value for --kv-cache-dtype"; kv_dtype="$2"; alias_setting_args+=(--kv-cache-dtype "$2"); bundle_explicit_run_args+=(--kv-cache-dtype "$2"); shift 2 ;;
      ...
      --)          shift; vllm_passthrough_args+=("$@"); break ;;
      ...
      -*)
        if [[ -n "$bundle_name" ]] && bundle_option_exists_in "$bundle_manifest" "${1#--}"; then
          ...
          vllm_passthrough_args+=("$1"); shift
        fi ;;
      *)           if [[ -z "$model" ]]; then model="$1"; else vllm_passthrough_args+=("$1"); fi; shift ;;
    esac
  done
  ...
  if [[ "${SPARK_ALIAS_BYPASS:-0}" != "1" ]] && alias_exists "$model"; then
    [[ "$alias_frozen_flags" == "0" ]] || die "Alias launch policy is fixed" \
      "--tools, --text-only, --no-reasoning, --mtp, --no-mtp, --no-mem-limit, --regen-profile, and --no-pull need: spark alias edit ${model}"
    cmd_alias_run "$model" \
      ${alias_setting_args[@]+"${alias_setting_args[@]}"} \
      ${vllm_passthrough_args[@]+"${vllm_passthrough_args[@]}"} \
      ${alias_override_args[@]+"${alias_override_args[@]}"}
    return 0
  fi
...
run_main_replacement_vllm() {
  ...
  if [[ "$old_provider" == "vllm" && -n "$old_model" ]]; then
    ...
    info "Stopping current LiteLLM main '${old_model}' before checking capacity for '${target_model}'"
    SPARK_SKIP_GATEWAY_REFRESH=1 stop_one_container "$old_name" || \
      die "Could not stop current main model '${old_model}'"
  fi
...
# Alias launch arguments can be replaced persistently (`spark alias set`) or for
# one launch (`spark run <alias> --kv-cache-memory-bytes 12G`). Spark settings
# and any vLLM argument are accepted; the helpers below define the exceptions.

# Launch policy flags that Spark expands into several vLLM arguments. Aliases
# keep them fixed; recreate the alias with `spark alias edit` to change them.
alias_policy_flag() {
  case "$1" in
    --tools|--text-only|--no-reasoning|--mtp|--no-mtp|--no-mem-limit|--regen-profile|--no-pull) return 0 ;;
    *) return 1 ;;
  esac
}

# Flags that only affect one launch; aliases never store them.
alias_run_only_flag() {
  case "$1" in
    --dry-run|--explain|--force|--tail|--no-wait|--main) return 0 ;;
    *) return 1 ;;
  esac
}

# vLLM flags that carry secrets or read external files; aliases never store them.
alias_forbidden_flag() {
  case "$1" in
    --api-key|--hf-token|--config|--ssl-keyfile|--ssl-keyfile-password) return 0 ;;
    *) return 1 ;;
  esac
}

# The flag an alias stores for a requested one. Captured launches keep vLLM
# names; guided and bundle aliases keep Spark's names for the settings Spark
# sizes memory with, so its launch plan stays accurate.
alias_stored_flag() {
  local kind="$1" flag="$2"
  if [[ "$kind" == "captured-vllm" ]]; then
    case "$flag" in
      --mem) flag=--gpu-memory-utilization ;;
      --max-len) flag=--max-model-len ;;
      --no-enforce-eager) flag=--enforce-eager ;;
    esac
  else
    case "$flag" in
      --gpu-memory-utilization) flag=--mem ;;
      --max-model-len) flag=--max-len ;;
    esac
  fi
  printf '%s\n' "$flag"
}

alias_setting_requires_value() {
  case "$1" in
    --mem|--gpu-memory-utilization|--max-len|--max-model-len|--max-num-seqs|--kv-cache-dtype|--port) return 0 ;;
    *) return 1 ;;
  esac
}

# Spark validates the settings it plans memory with; vLLM validates the rest.
alias_validate_setting() {
  local flag="$1" value="$2"
  case "$flag" in
    --mem|--gpu-memory-utilization)
      [[ "$value" =~ ^(0?[.][0-9]+|1([.]0+)?)$ ]] \
        && awk -v v="$value" 'BEGIN { exit !(v > 0 && v <= 1) }' \
        || die "Invalid value for ${flag}: ${value}" "Use a GPU memory fraction greater than 0 and at most 1."
      ;;
    --max-len|--max-model-len|--max-num-seqs|--port)
      [[ "$value" =~ ^[1-9][0-9]*$ ]] || die "Invalid value for ${flag}: ${value}" "Use a positive integer."
      ;;
    --kv-cache-dtype)
      case "$value" in
        auto|bfloat16|fp8) ;;
        *) die "Invalid value for --kv-cache-dtype: ${value}" "Use auto, bfloat16, or fp8." ;;
      esac
      ;;
  esac
}

# Print the state of one flag in an args array: nothing when absent, "on" for
# a bare switch, or "=<value>" for `--flag value` and `--flag=value`. A token
# after the flag that is not itself a flag is that flag's value.
alias_args_flag_state() {
  jq -r --arg flag "$2" '
    . as $args
    | [range(0; length) as $i
        | if $args[$i] == $flag then
            (if $i + 1 < ($args | length) and ($args[$i + 1] | startswith("--") | not)
             then "=" + $args[$i + 1] else "on" end)
          elif ($args[$i] | startswith($flag + "=")) then "=" + ($args[$i] | ltrimstr($flag + "="))
          else empty end]
    | last // empty
  ' <<<"$1"
}

# Print an args array without one flag and its value, in either form.
alias_args_remove_json() {
  jq -c --arg flag "$2" '
    . as $args
    | reduce range(0; length) as $i ({out: [], skip: false};
        if .skip then .skip = false
        elif $args[$i] == $flag then
          .skip = ($i + 1 < ($args | length) and ($args[$i + 1] | startswith("--") | not))
        elif ($args[$i] | startswith($flag + "=")) then .
        else .out += [$args[$i]] end)
    | .out
  ' <<<"$1"
...
# `--no-foo` and `--foo` are one boolean; setting one removes the other.
alias_opposite_switch() {
  case "$1" in
    --no-*) printf -- '--%s\n' "${1#--no-}" ;;
    *)      printf -- '--no-%s\n' "${1#--}" ;;
  esac
...
# Print the flags a list of alias arguments touches, in order.
alias_settings_flags() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --*=*)   printf '%s\n' "${1%%=*}"; shift ;;
      --*)
...
# Print a definition with launch arguments replaced or added (`--flag value`,
# `--flag=value`, `--switch`). Guided and bundle
# aliases keep them in run_args, which Spark forwards to vLLM when it does not
# own the flag; captured launches keep the exact vLLM command in vllm_args.
alias_apply_settings() {
  local definition="$1" kind key args flag value has_value target state
  shift
  kind=$(jq -r '.kind' <<<"$definition")
  case "$kind" in
    guided|bundle) key="run_args" ;;
    captured-vllm) key="vllm_args" ;;
    *) die "Alias kind '${kind}' does not support argument changes" ;;
  esac
  args=$(jq -c --arg key "$key" '.[$key] // []' <<<"$definition")
  while [[ $# -gt 0 ]]; do
    has_value=0 value=""
    case "$1" in
      --*=*) flag="${1%%=*}"; value="${1#*=}"; has_value=1; shift ;;
      --*)
        flag="$1"; shift
        if [[ $# -gt 0 && "$1" != --* ]]; then value="$1"; has_value=1; shift; fi
        ;;
      *) die "Unexpected argument: $1" "Pass arguments as --flag value, --flag=value, or --flag" ;;
    esac
    if alias_run_only_flag "$flag"; then
      die "${flag} only applies to one launch" "Use it with: spark run <alias> ${flag}"
    fi
    if alias_policy_flag "$flag"; then
      die "${flag} is fixed by the alias" "Recreate the alias with: spark alias edit <alias>"
    fi
    target=$(alias_stored_flag "$kind" "$flag")
    if alias_forbidden_flag "$target"; then
      die "${flag} cannot be used with an alias" "Aliases never store or replay secrets or external config files."
    fi
    if alias_setting_requires_value "$target"; then
      [[ "$has_value" == "1" ]] || die "Missing value for ${flag}"
      alias_validate_setting "$target" "$value"
    fi
    # A captured launch runs vLLM directly, where CUDA graphs are the default.
    if [[ "$kind" == "captured-vllm" && "$flag" == "--no-enforce-eager" ]]; then
      [[ "$has_value" == "0" ]] || die "--no-enforce-eager takes no value"
      args=$(alias_args_remove_json "$args" "$target")
      continue
    fi
    state=$(alias_args_flag_state "$args" "$target")
    # An unchanged argument keeps its position, so re-applying it is a no-op.
    if [[ "$has_value" == "1" && "$state" == "=${value}" ]] || [[ "$has_value" == "0" && "$state" == "on" ]]; then
      continue
    fi
    [[ "$has_value" == "1" ]] || args=$(alias_args_remove_json "$args" "$(alias_opposite_switch "$target")")
    args=$(alias_args_remove_json "$args" "$target")
    if [[ "$has_value" == "1" ]]; then
      args=$(jq -c --arg flag "$target" --arg value "$value" '. + [$flag, $value]' <<<"$args")
    else
      args=$(jq -c --arg flag "$target" '. + [$flag]' <<<"$args")
    fi
  done
  jq -c --arg key "$key" --argjson args "$args" '.[$key] = $args' <<<"$definition"
}

# Print how an alias stores one flag, for change summaries.
alias_setting_display() {
  local definition="$1" flag="$2" kind key state
  kind=$(jq -r '.kind' <<<"$definition")
  [[ "$kind" == "captured-vllm" ]] && key="vllm_args" || key="run_args"
  state=$(alias_args_flag_state "$(jq -c --arg key "$key" '.[$key] // []' <<<"$definition")" \
    "$(alias_stored_flag "$kind" "$flag")")
  case "$state" in
    "") printf 'default\n' ;;
    on) printf 'on\n' ;;
    *)  printf '%s\n' "${state#=}" ;;
  ...
  case "$kind" in
    ...
    captured-vllm)
      ...
      while IFS= read -r arg; do
        flag="${arg%%=*}"
        if alias_forbidden_flag "$flag"; then return 1; fi
...
cmd_alias_run() {
  ...
  shift
  local -a overrides=() settings=()
  # Operational flags drive this launch; every other argument changes the
  # alias's launch arguments for this launch only.
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --port)
      ...
      *)
        if alias_run_only_flag "$1"; then overrides+=("$1"); else settings+=("$1"); fi
        shift ;;
    esac
  done
  ...
  if [[ ${#settings[@]} -gt 0 ]]; then
    definition=$(alias_apply_settings "$definition" "${settings[@]}") || exit 1
    alias_validate_definition "$definition" || die "One-off settings make alias '${name}' invalid"
    info "One-off arguments for this run: ${settings[*]} (alias '${name}' unchanged)"
  fi
  backend=$(jq -r '.backend' <<<"$definition")
  kind=$(jq -r '.kind' <<<"$definition")
  case "$kind" in
    guided)
      ...
      cmd_run "$model" ${run_args[@]+"${run_args[@]}"}
      ;;
    captured-vllm)
      ...
      run_captured_vllm_definition "$definition" "Alias '${name}'" ${overrides[@]+"${overrides[@]}"}
      ;;
    bundle)
...
cmd_alias() {
  ...
  case "$action" in
    ...
    set)
      name="${1:-}"
      [[ -n "$name" && $# -ge 2 ]] || die "Usage: spark alias set <alias> <arguments...>" \
        "Spark settings (--mem, --max-len, --max-num-seqs, --kv-cache-dtype, --port, --enforce-eager) or any vLLM argument"
      shift
      local current updated flag before after
      current=$(alias_definition "$name") || die "Alias '${name}' does not exist"
      updated=$(alias_apply_settings "$current" "$@") || exit 1
      if jq -e --argjson a "$current" --argjson b "$updated" -n '$a == $b' >/dev/null; then
        info "Alias '${name}' already uses these arguments"
        return 0
      fi
      alias_save_definition "$name" "$updated" 1
      info "Updated alias '${name}' (previous copy: spark alias rollback ${name})"
      while IFS= read -r flag; do
        before=$(alias_setting_display "$current" "$flag")
        after=$(alias_setting_display "$updated" "$flag")
        [[ "$before" == "$after" ]] || printf '    %s: %s → %s\n' "$flag" "$before" "$after"
      done < <(alias_settings_flags "$@")
      ;;
    capture)
  ...
  ${BOLD}Usage:${NC} spark alias <command>
    ...
    set <alias> <arguments...>
                              Replace or add launch arguments and keep the
                              rest: Spark settings or any vLLM argument
  ...
  Change arguments for one run only: spark run <alias> --kv-cache-memory-bytes 12G
