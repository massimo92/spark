...
cmd_run_help() {
  ...
  Aliases keep other model settings frozen. They accept operational overrides
  (--dry-run, --explain, --force, --port, --tail, --no-wait, --main) and
  one-off --mem, --max-len, --max-num-seqs, and --kv-cache-dtype values that
  apply to that run only. Save a change with: spark alias set <alias> ...
...
cmd_run() {
  ...
  if [[ "${SPARK_ALIAS_BYPASS:-0}" != "1" ]] && alias_exists "$model"; then
    [[ "$alias_frozen_flags" == "0" ]] || die "Alias model settings are frozen" \
      "Allowed overrides: --dry-run, --explain, --force, --port, --tail, --no-wait, --main, --mem, --max-len, --max-num-seqs, --kv-cache-dtype"
    cmd_alias_run "$model" \
...
# Model settings an alias can change persistently (`spark alias set`) or for a
# single launch (`spark run <alias> --mem 0.6`). Captured launches store the
# equivalent vLLM flag instead of Spark's flag.
alias_setting_vllm_flag() {
  case "$1" in
    --mem)            printf '%s\n' --gpu-memory-utilization ;;
    --max-len)        printf '%s\n' --max-model-len ;;
    --max-num-seqs)   printf '%s\n' --max-num-seqs ;;
    --kv-cache-dtype) printf '%s\n' --kv-cache-dtype ;;
    *) return 1 ;;
  esac
}
  ...
  case "$flag" in
    --mem)
      [[ "$value" =~ ^(0?[.][0-9]+|1([.]0+)?)$ ]] \
        ...
        || die "Invalid value for --mem: ${value}" "Use a GPU memory fraction greater than 0 and at most 1."
      ;;
    --max-len|--max-num-seqs)
...
# Print the stored value of one setting, reading `--flag value` or `--flag=value`.
alias_setting_value() {
  local definition="$1" flag="$2" kind key
  kind=$(jq -r '.kind' <<<"$definition")
  if [[ "$kind" == "captured-vllm" ]]; then
    key="vllm_args"; flag=$(alias_setting_vllm_flag "$flag") || return 1
  else
    key="run_args"
  fi
  jq -r --arg key "$key" --arg flag "$flag" '
    (.[$key] // []) as $args
    | [range(0; $args | length) as $i
        | if $args[$i] == $flag and $i + 1 < ($args | length) then $args[$i + 1]
...
# Print a definition with model settings replaced. Arguments are Spark
# flag/value pairs; guided and bundle aliases keep them in run_args, captured
# launches in vllm_args under the equivalent vLLM flag.
alias_apply_settings() {
  local definition="$1" kind flag value target key args
  shift
  kind=$(jq -r '.kind' <<<"$definition")
  case "$kind" in
    guided|bundle) key="run_args" ;;
    captured-vllm) key="vllm_args" ;;
    *) die "Alias kind '${kind}' does not support setting changes" ;;
  esac
  while [[ $# -gt 0 ]]; do
    flag="$1"
    target=$(alias_setting_vllm_flag "$flag") \
      || die "Unsupported alias setting: ${flag}" "Supported: --mem, --max-len, --max-num-seqs, --kv-cache-dtype"
    [[ $# -ge 2 ]] || die "Missing value for ${flag}"
    value="$2"
    alias_validate_setting "$flag" "$value"
    # An unchanged value keeps its position, so re-applying a setting is a no-op.
    if [[ "$(alias_setting_value "$definition" "$flag")" == "$value" ]]; then
      shift 2
      continue
    fi
    [[ "$key" == "vllm_args" ]] || target="$flag"
    args=$(alias_vllm_set_value_json "$(jq -c --arg key "$key" '.[$key] // []' <<<"$definition")" "$target" "$value") \
      || die "Cannot update ${flag}"
    definition=$(jq -c --arg key "$key" --argjson args "$args" '.[$key] = $args' <<<"$definition")
    shift 2
  done
  printf '%s\n' "$definition"
}
...
cmd_alias_run() {
  ...
  # Model settings apply to this launch only; everything else is operational.
  while [[ $# -gt 0 ]]; do
    if alias_setting_vllm_flag "$1" >/dev/null; then
      [[ $# -ge 2 ]] || die "Missing value for $1"
      settings+=("$1" "$2"); shift 2
    else
      overrides+=("$1"); shift
    fi
  done
  ...
  if [[ ${#settings[@]} -gt 0 ]]; then
    ...
    info "One-off settings for this run: ${settings[*]} (alias '${name}' unchanged)"
  fi
  backend=$(jq -r '.backend' <<<"$definition")
  kind=$(jq -r '.kind' <<<"$definition")
  case "$kind" in
...
cmd_alias() {
  ...
  case "$action" in
    ...
    set)
      name="${1:-}"
      [[ -n "$name" && $# -ge 2 ]] || die "Usage: spark alias set <alias> <flag> <value> [<flag> <value>...]" \
        "Flags: --mem, --max-len, --max-num-seqs, --kv-cache-dtype"
      shift
      ...
      if jq -e --argjson a "$current" --argjson b "$updated" -n '$a == $b' >/dev/null; then
        info "Alias '${name}' already uses these settings"
        return 0
      fi
      ...
      while [[ $# -gt 0 ]]; do
        flag="$1"
        before=$(alias_setting_value "$current" "$flag")
        after=$(alias_setting_value "$updated" "$flag")
        [[ "$before" == "$after" ]] || printf '    %-17s %s → %s\n' "$flag" "${before:-default}" "$after"
        shift 2
      done
      ;;
    capture)
  ...
  ${BOLD}Usage:${NC} spark alias <command>
    ...
    set <alias> <flag> <value>...
                              Replace model settings and keep the rest:
                              --mem, --max-len, --max-num-seqs, --kv-cache-dtype
  ...
  Change a setting for one run only: spark run <alias> --mem 0.6
