...
cmd_bundle_remove() {
  [[ $# -gt 0 ]] || die "Usage: spark bundle remove <name> [<name>...]"
  local candidate existing duplicate=0 name path label="" i
  local -a names=() paths=()

  for candidate in "$@"; do
    is_safe_bundle_name "$candidate" || die "Invalid bundle name: ${candidate}"
    duplicate=0
    if (( ${#names[@]} > 0 )); then
    ...
    path="${BUNDLES_DIR}/imported/${candidate}"
    [[ -d "$path" ]] || die "Imported bundle '${candidate}' does not exist" "Built-in bundles cannot be removed."
    names+=("$candidate")
    paths+=("$path")
  done
  ...
  if (( ${#names[@]} == 1 )); then
    confirm "Remove imported bundle '${names[0]}'?" || { warn "Cancelled"; return 0; }
  else
    confirm "Remove ${#names[@]} imported bundles (${label})?" || { warn "Cancelled"; return 0; }
  fi

  for i in "${!paths[@]}"; do
    name="${names[$i]}"
    rm -rf -- "${paths[$i]}" || die "Cannot remove imported bundle '${name}'"
  done
  if (( ${#names[@]} == 1 )); then
    info "Removed bundle '${names[0]}'"
  else
    info "Removed ${#names[@]} bundles: ${label}"
  fi
}
...
cmd_bundle_help() {
  ...
  ${BOLD}Usage:${NC} spark bundle [command]
    ...
    remove <name>...               Remove one or more imported bundles
